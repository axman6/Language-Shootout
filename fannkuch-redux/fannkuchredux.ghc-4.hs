...
main = do
    var <- newEmptyMVar
    forkIO (master var)
    -- forkOnIO numCapabilities (master var)
    takeMVar var
...
master var = do
   ...
   let bk = fact `quot` num
   forM_ [0,bk..fact-1] $ \ ix -> do
    forkIO (unrank ix n $ \ p -> genPermutations n ix (min fact (ix + bk)) p >=> writeChan chan)
   F chksm mflops <- go chan 0 num mempty
